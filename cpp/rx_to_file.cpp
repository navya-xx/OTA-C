                  ...
                  bool continue_on_bad_packet = false)
{
    ...
    while (not stop_signal_called and (num_requested_samples != num_total_samps or num_requested_samples == 0) and (time_requested == 0.0 or std::chrono::steady_clock::now() <= stop_time))
    {
    ...
    if (stats)
    {
        ...
        std::cout << boost::format("Received %d samples in %f seconds") % num_total_samps % actual_duration_seconds
                  << std::endl;
...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
    ...
    desc.add_options()
        ...
        ("gain", po::value<double>(&gain)->default_value(89.8), "gain for the RF chain")
