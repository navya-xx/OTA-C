...
class CSD_upsamp              // class implementing cycle start detector using ZFC seq as reference
{
private:
    ...
    size_t upsamp_factor;                     // Upsampling factor at the Tx
...
public:
    CSD_upsamp(size_t capacity, uhd::time_spec_t sample_duration, size_t N_zfc, size_t m_zfc, size_t R_zfc, size_t upsamp_factor, float rx_noise_power, float pnr_threshold) : samples_buffer(capacity), save_buffer(N_zfc * (R_zfc + 2), samp_type(0.0, 0.0)), successful_detection(false), timer(capacity), num_samp_corr(N_zfc * upsamp_factor), sample_duration(sample_duration), capacity(capacity), front(0), rear(0), num_produced(0), N_zfc(N_zfc), m_zfc(m_zfc), R_zfc(R_zfc), upsamp_factor(upsamp_factor), peaks_count(0), pnr_threshold(pnr_threshold), peak_indices(R_zfc), peak_vals(R_zfc), peak_times(R_zfc), rx_noise_power(rx_noise_power) {}
    ...
    int correlation_operation()
    {
        ...
        for (size_t i = 0; i < upsamp_factor; ++i)
        {
            ...
            {
                corr += samples_buffer[(front + i + (j * upsamp_factor)) % capacity] * std::conj(zfc_seq[j]);
            }
...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
    ...
    size_t channel, spb, capacity_mul, max_sample_size, num_samp_corr, Rx_N_zfc, Tx_N_zfc, Rx_m_zfc, Tx_m_zfc, R_zfc, rx_upsampling_factor;
    ...
    desc.add_options()
        ...
        ("Rx-upsamp", po::value<size_t>(&rx_upsampling_factor)->default_value(10), "Rx upsampling multiplier.")
    ...
    if (num_samp_corr == 0)
    {
        num_samp_corr = Rx_N_zfc * rx_upsampling_factor;
    }

    size_t capacity = capacity_mul * num_samp_corr;

    // create class object -> buffer handler
    CSD_upsamp<std::complex<float>> csdbuffer(capacity, sample_duration, Rx_N_zfc, Rx_m_zfc, R_zfc, rx_upsampling_factor, noise_power, pnr_threshold);
