...
public:
    ...
    size_t samples_from_first_peak = 0;
    ...
    int correlation_operation()
    {
        ...
        float noise_level = std::sqrt(rx_noise_power);
    ...
    std::tuple<bool, uhd::time_spec_t, float> consume()
    {
        ...
        if (DEBUG)
        {
            std::string lock_condition = (num_produced >= num_samp_corr * 2) ? "true" : "false";
...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
    ...
    desc.add_options()
        ...
        ("file", po::value<std::string>(&file), "name of the file to write binary samples to")
        ...
        ("Rx-N-zfc", po::value<size_t>(&Rx_N_zfc)->default_value(79), "Rx ZFC seq length.")
        ...
        ("pnr-threshold", po::value<double>(&pnr_threshold)->default_value(2.5), "Peak to noise ratio threshold for detecting peaks.")
    ;
    // clang-format on
    po::variables_map vm;
    ...
    auto rx_producer_thread = thread_group.create_thread([=, &csdbuffer, &stop_thread_signal]()
        ...
        while (not stop_signal_called and not stop_thread_signal and not (total_time > 0 and std::chrono::steady_clock::now() > stop_time))
        {
