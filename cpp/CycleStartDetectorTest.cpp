...
private:
    ...
    size_t pnr_threshold;                     // Threshold to detect peak
...
public:
    ...
    size_t curr_pnr_threshold = pnr_threshold;
        ...
        for (size_t i = 0; i < num_samp_corr; ++i)
        {
            ...
            if (peak_to_noise_ratio > curr_pnr_threshold)
            {
                ...
                if (peaks_count <= R_zfc) // fill in the first peak
                {
                    ...
                    {
                        peak_indices[0] = i;
                        peak_vals[0] = peak_to_noise_ratio;
                    ...
                    {
                        last_peak = peak_indices[peaks_count - 1];
                        last_peak_val = peak_vals[peaks_count - 1];
                        auto max_peak_val_it = std::max_element(peak_vals.begin(), peak_vals.end());
                        curr_pnr_threshold = static_cast<size_t>(*max_peak_val_it * 0.9);
                        ...
                        {
                            if (last_peak_val < peak_to_noise_ratio) // check is this peak is higher than the previous
                            {
                                // update last peak
                                peak_indices[peaks_count - 1] = i;
                                peak_vals[peaks_count - 1] = peak_to_noise_ratio;
                        ...
                        else // peak found in correct location
                        {
                            if (peaks_count < R_zfc)
                            {
                                ...
                                peak_vals[peaks_count] = peak_to_noise_ratio;
...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
    ...
    CycleStartDetector<std::complex<float>> csdbuffer(capacity, sample_duration, num_samp_corr, N_zfc, m_zfc, R_zfc, noise_power, pnr_threshold);
    ...
    // CSD consumer thread
    auto rx_consumer_thread = thread_group.create_thread([=, &csdbuffer, &csd_detect_time, &stop_thread_signal]()
                                                         {
            // keep consuming
            const auto start_time = std::chrono::steady_clock::now();
            ...
            int64_t time_micro = static_cast<int64_t>((usrp_now.get_real_secs() - result.second.get_real_secs()) * 1e6);
            csd_detect_time = result.second;
    ...
    // -----------------------------------------------------------------------------------------
    // start transmission
    // estimation of channel power -- later
    ...
    txmd.time_spec = csd_detect_time + uhd::time_spec_t(TX_WAIT_TIME_MICROSECS / 1e6);
    ...
    std::vector<std::complex<float>> tx_zfc_seq = csdbuffer.generateZadoffChuSequence(721, 31);

    size_t total_num_samps = tx_zfc_seq.size(); // only transmit a single zfc seq
    ...
    while (true)
    {
        ...
        num_acc_samps += tx_stream->send(&csdbuffer.zfc_seq, total_num_samps, txmd);
    ...
    std::cout << "Transmission of one ZFC(\"721, 31\") seq over!" << std::endl;
