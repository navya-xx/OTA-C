...
do
    ...
    tmux send-keys -t $SESSION_NAME:0.$i  "ssh ${node_name} 'cd \$HOME/OTA-C && git stash && git checkout main && git pull && mkdir -p \$HOME/OTA-C/ProjectRoot/build/ && cd \$HOME/OTA-C/ProjectRoot/build/ && cmake ../ && make -j && echo \"Success\"'" C-m
